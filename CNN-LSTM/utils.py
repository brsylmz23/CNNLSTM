...
def CrossValidation(gettingfiles, stat_id, **kwargs):
    ...
    def is_last_four_digits(file_name):
        return file_name[-8:-4].isdigit()
    
    # Files whose last 4 characters are digits
    digit_files = [i for i, file in enumerate(gettingfiles) if is_last_four_digits(file)]
    non_digit_files = [i for i, file in enumerate(gettingfiles) if not is_last_four_digits(file)]
    ...
    if kwargs.get('Crossvalidation_type') == "Chronological":
        ...
        rng = default_rng()
        num_trainval = len(digit_files)
        num_train = int((num_trainval * train_percent) / 100)
        num_val = num_trainval - num_train
        
        trainval_indices = rng.choice(digit_files, size=num_trainval, replace=False)
        
        trind = trainval_indices[:num_train].tolist()
        vlind = trainval_indices[num_train:].tolist()
        tsind = non_digit_files
        ...
        # Ensure the split matches the provided percentages
        total_digit_files = len(digit_files)
        num_train = int(total_digit_files * train_percent / 100)
        num_val = total_digit_files - num_train
        
        for station, indices in stat_id_dict.items():
            digit_indices = [i for i in indices if i in digit_files]
            if len(trind) < num_train:
                remaining_train_space = num_train - len(trind)
                trind.extend(digit_indices[:remaining_train_space])
                digit_indices = digit_indices[remaining_train_space:]
            if len(vlind) < num_val:
                remaining_val_space = num_val - len(vlind)
                vlind.extend(digit_indices[:remaining_val_space])
                digit_indices = digit_indices[remaining_val_space:]
            tsind.extend([i for i in indices if i in non_digit_files])  # Add non-digit indices to test
...
#         rng = default_rng()
#         trainval = rng.choice(int((len(gettingfiles) * int(kwargs.get('Train percentage'))) / 100), 
#                               int((len(gettingfiles) * int(kwargs.get('Train percentage'))) / 100), replace=False)
#         test = [x for x in range(int((len(gettingfiles) * int(kwargs.get('Train percentage'))) / 100), len(gettingfiles))]
#         # Store train-test-val indexes to trind, tsind, vlind    
#         trind = trainval[:int((len(gettingfiles) * val_percent) / 100)]
#         vlind = trainval[int((len(gettingfiles) * val_percent) / 100):int((len(gettingfiles) * int(kwargs.get('Train percentage'))) / 100)]
...
#         # Read matching names from the file
#         with open(r"D:\Baris\codes\baris-git06\liste.txt", "r") as dosya:
...
#         # Add indices of matching names to tsind
#         for station, indices in stat_id_dict.items():
#             for i in indices:
#                 if gettingfiles[i] in eslesen_isimler:
#                     tsind.append(i)

#         # Add indices of non-matching names to trind and vlind
#         for station, indices in stat_id_dict.items():
#             for i in indices:
#                 if gettingfiles[i] not in eslesen_isimler:
#                     if len(vlind) < (len(gettingfiles) * 0.02):
#                         vlind.append(i)
#                     else:
#                         trind.append(i)
...
def loader_helper(b, signal, alt, mag, numofData, epicenterLat, epicenterLon, epicenterDepth, 
    ...
    try: 
        ...
        if numofData < 6000:
            new_shape = (6000, 3)
            # Yeni dizi oluştur ve sıfırlarla doldur
            padded_signal = np.zeros(new_shape, dtype=signal.dtype)
            
            # Mevcut signal verilerini yeni dizinin sonuna kopyala
            padded_signal[-signal.shape[0]:, :] = signal
            
            signal = padded_signal
            
            numofData = signal.shape[0]
                ...
                if numofData > (int(kwargs.get('signaltime'))*kwargs.get('fs'))-1:
                    # Check if epicenter depth is less than desired depth value
                     if epicenterDepth < float(kwargs.get('depth')):
                         ...
                         start_ind = int(median_index - (kwargs.get('signaltime')/2))
                         if start_ind <0 :
                             start_ind=0
                         end_ind = start_ind + kwargs.get('signaltime')
                         
                         if end_ind > (len(signal)/kwargs.get('fs')):
                             end_ind = len(signal)/kwargs.get('fs')
                             
                         signal = signal[start_ind*kwargs.get('fs'):end_ind*kwargs.get('fs')]
